...
export function Shell({ children }: { children: ReactNode }) {
  ...
  const visibleNav = NAV.filter((n) => {
    ...
    if (n.k === "/leads") return app.role === "admin" || app.role === "co_admin" || app.role === "manager" || app.role === "staff";
    if (n.k === "/classes") return app.role === "admin" || app.role === "manager" || app.role === "co_admin";
    if (n.k === "/payroll") return can.payroll;
    if (n.k === "/import") return app.role === "admin" || app.role === "co_admin";
    if (n.k === "/invoices") return can.viewInvoices;
    if (n.k === "/records") return can.addRecords || app.role === "admin";
    if (n.k === "/reports") return can.approve || app.role === "admin" || app.role === "co_admin";
